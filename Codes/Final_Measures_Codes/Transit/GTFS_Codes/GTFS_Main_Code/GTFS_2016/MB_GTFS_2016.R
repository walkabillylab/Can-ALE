...
setwd("V:/CanALE/Data/GTFS/GTFS_2016/MB")


# Base URL for MB Transit GTFS on TransitFeeds
base_url <- "https://transitfeeds.com/p/winnipeg-transit/23"
...
is_valid_date <- function(date) {
  ...
  holidays <- as.Date(c(
    "2016-02-15", # Louis Riel Day (similar to Family Day)
    "2016-03-25", # Good Friday
    "2016-03-28", # Easter Monday (not statutory, but excluded)
    "2016-05-23", # Victoria Day
    "2016-07-01", # Canada Day
    "2016-08-01", # Civic Holiday (Terry Fox Day, not statutory)
  ...
  for (date in date_range) {
    ...
    if (is_valid_date(date)) {
      ...
      if (!is.null(response) && response$status_code == 200) {
        ...
        local_file <- file.path(date_folder, "Winnipeg.zip")
