...
setwd("V:/CanALE/Data/GTFS/GTFS_2016/NS")

# Base URL for NS Transit GTFS on TransitFeeds
base_url <- "https://transitfeeds.com/p/metrotransit/36"
  ...
  for (date in date_range) {
    ...
    if (is_valid_date(date)) {
      ...
      if (!is.null(response) && response$status_code == 200) {
        ...
        local_file <- file.path(date_folder, "metrotransit.zip")
