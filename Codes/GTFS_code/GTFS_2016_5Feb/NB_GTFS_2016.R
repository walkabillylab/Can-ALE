...
setwd("V:/CanALE/Data/GTFS/GTFS_2016/NB")

# Base URL for NB Transit GTFS on TransitFeeds
base_url <- "https://transitfeeds.com/p/codiac-transpo/829"
...
is_valid_date <- function(date) {
  ...
  holidays <- as.Date(c(
    ...
    "2016-03-28", # Easter Monday (not statutory, but excluded if desired)
    "2016-05-23", # Victoria Day
    "2016-07-01", # Canada Day
    "2016-08-01", # New Brunswick Day
...
date_ranges <- list(
  ...
  seq(as.Date("2016-02-01"), as.Date("2016-02-29"), by = "day"), # 2016 is a leap year
  ...
  for (date in date_range) {
    ...
    if (is_valid_date(date)) {
      ...
      if (!is.null(response) && response$status_code == 200) {
        ...
        local_file <- file.path(date_folder, "Codiac Transpo.zip")
...
# Base URL #
base_url <- "https://transitfeeds.com/p/saint-john-transit/1224"
  ...
  for (date in date_range) {
    ...
    if (is_valid_date(date)) {
      ...
      if (!is.null(response) && response$status_code == 200) {
        ...
        # Save the GTFS file in the date-specific folder
        local_file <- file.path(date_folder, "Saint John.zip")
        download.file(gtfs_url, local_file, mode = "wb")
